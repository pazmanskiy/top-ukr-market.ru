(function(window) {
    ...
    Scripts.prototype.initialize = function() {
        ...
        script.onload = function() {
            // create custom event on script load
            ...
            if (typeof GeoInfo != 'undefined' && typeof lCountries != 'undefined') {
                lCountries.hasOwnProperty('updateCurrentCityHelper') && lCountries.updateCurrentCityHelper(window.GeoInfo.city);
                lCountries.hasOwnProperty('nginxCountryCode') && (lCountries.nginxCountryCode = GeoInfo.countryCode.toLowerCase());
                lCountries.hasOwnProperty('defaultCountry') && (lCountries.defaultCountry = GeoInfo.countryCode.toLowerCase());
                

                // init and dispatch countryset event for external usage 
                customEvent.initEvent('countryset', true, false);
    ...
    Shakes.prototype.orderConfirm = function(data) {
        ...
        var langCode = lCountries.userCountryCode ? lCountries.userCountryCode : 'ru';
        ...
        if (this.config.orderRedirect !== '') {
            this.config.orderRedirect = Requests('orderRedirect').replace('%requestID%', data.request_id).replace('%langCode%.', placeholder);
    ...
    if (Helpers.getCookie('parking') == 1) {
        // Сценарий паркинга
        ...
        window.lShakes = new Shakes({
            ...
            cookie_domains: ['shak' + 'eshakepoto' + 'k.com', 'seka' + 'hspot' + 'ok.com', 'sha' + 'kepo' + 'tok.com', 'r.sha' + 'kesstream.com', 'sha' + 'kesstream.com', 'new' + 'good' + 'sshake.com', 'goods' + 'shake.com', 'shak' + 'esspot.com'],
            comeBacker: {
                status: true,
            },
        });
