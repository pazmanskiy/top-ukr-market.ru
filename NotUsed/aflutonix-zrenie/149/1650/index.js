...
function mouseMoved() {
	...
	$('.mouse_moved').mouseover(function () {
		if( flag == true){
			var displayPop = $(".popup-first").css('display');
			var displayP = $(".phone_modal").css('display');
...
//31/05/16 M.Vovk
//Добавил переменную, котороя определяет путь к картинкам..Теперь, на попапы с заказами, нужно добавить строчку вниз html документа:
//<img src="img/" alt="" class="take" style="display:none;">
// добавлять другой код не нужно, скрипт сам все добавит
//
//Также, сделал так что теперь не нужно добавлять ничего в html, для добавления синей трубки для попапов
//
//Убрал со скрипта, возможность, подключать отдельные стили для оперы мини и Сафари под винду. 
//
//Скрипт на выход мышки тоже поправил, так же в html не нужно ничего добавлять, просто включить в скрипте(сменив, соответствующее поле false на true
//------------------------------------------------------------------------------

//17/06/16 M.Vovk
//Больше не нужно подключать картинки, все картинки загнаны в Base64 
//Попапы с заказами, теперь включаем только в js файле, в HTML ничего прописывать не нужно.. 
//------------------------------------------------------------------------------

//24/06/16 kedrovskiy
// иконки перегнал в спрайт и вывел стилями
...
$(document).ready(function () {
    ...
    $('.modal-first, .phone_close').on('click', function () {
        $(".phone_modal").fadeOut();
    });


    $('.modal_mouse_pop, .mous_close').on('click', function () {
        $('.modal_mouse_out').hide()
    });
    $('.modal_mouse_pop, .mous_close').on('click', function () {
        $('.modal_mouse_out').addClass('hide_forever')
    });
});
...
$('.modal-first, .mod_clouse').on('click', function () {
...
$('.full_ser').on('click', function (event) {
    $('.modal_img_one').fadeIn(400, function () {
        $('.modal_img').animate({opacity: 1}, 200);
    });
    event.preventDefault();
});

$('.mod_back').on('click', function () {
    $('.modal_img_one').fadeOut();


});


$('.full_ser_two').on('click', function (event) {
    $('.modal_img_two').fadeIn(400, function () {
        $('.modal_img').animate({opacity: 1}, 200);
    });
    event.preventDefault();
});
