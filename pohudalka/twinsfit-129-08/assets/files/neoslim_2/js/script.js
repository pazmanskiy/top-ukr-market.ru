...
$(document).ready(function () {
    ...
    $('.calc-button').on('click', function () {
        var weightNow = parseInt($('.calc-weight').val()),
            weightWant = parseInt($('.calc-want').val()),
        ...
        if (weightNow > weightWant) {
            weeks = Math.floor(weightNow - weightWant) / 5 + 1;
            month = weeks / 4;
            if (weeks >= 12) {
                resultText = 'Более 2 месяцев';
                lssCount = 9;
            }
            if (weeks >= 8 && weeks <= 12) {
                resultText = '2 мес.';
            ...
            if (weeks >= 4 && weeks < 8) {
                resultText = parseInt(month) + ' мес.';
                lssCount = 5;
            }
            if (weeks < 4) {
                resultText = parseInt(weeks) + ' нед.';
            }

            $('.weightNow-span').text(weightNow);
            $('.weightWant-span').text(weightWant);
            resultTime.text(resultText);
            $('.recomend-block p').show(0);
            $('.lssCount-block').text(lssCount);
        } else {
